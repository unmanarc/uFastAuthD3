...
bool IdentityManager_DB::Roles_DB::addRole(const std::string &roleName, const std::string &roleDescription)
{
    ...
    return _parent->m_sqlConnector->query("INSERT INTO iam_roles (`roleName`,`roleDescription`) VALUES(:roleName,:roleDescription);",
...
{
    Threads::Sync::Lock_RW lock(_parent->m_mutex);
    return _parent->m_sqlConnector->query("DELETE FROM iam_roles WHERE `roleName`=:roleName;", {{":roleName", MAKE_VAR(STRING, roleName)}});
}

bool IdentityManager_DB::Roles_DB::doesRoleExist(const std::string &roleName)
{
    Threads::Sync::Lock_RD lock(_parent->m_mutex);
    std::shared_ptr<SQLConnector::QueryInstance> i = _parent->m_sqlConnector->qSelect("SELECT `roleName` FROM iam_roles WHERE `roleName`=:roleName;", {{":roleName", MAKE_VAR(STRING, roleName)}}, {});
...
{
    Threads::Sync::Lock_RW lock(_parent->m_mutex);
    return _parent->m_sqlConnector->query("UPDATE iam_roles SET `roleDescription`=:roleDescription WHERE `roleName`=:roleName;",
                                          {{":roleName", MAKE_VAR(STRING, roleName)}, {":roleDescription", MAKE_VAR(STRING, roleDescription)}});
}
...
std::string IdentityManager_DB::Roles_DB::getRoleDescription(const std::string &roleName)
{
    ...
    std::shared_ptr<SQLConnector::QueryInstance> i = _parent->m_sqlConnector->qSelect("SELECT `roleDescription` FROM iam_roles WHERE `roleName`=:roleName LIMIT 1;",
...
std::set<std::string> IdentityManager_DB::Roles_DB::getRolesList()
{
    ...
    std::shared_ptr<SQLConnector::QueryInstance> i = _parent->m_sqlConnector->qSelect("SELECT `roleName` FROM iam_roles;", {}, {&roleName});
...
std::list<RoleDetails> IdentityManager_DB::Roles_DB::searchRoles(std::string sSearchWords, size_t limit, size_t offset)
{
    ...
    std::string sSqlQuery = "SELECT `roleName`,`roleDescription` FROM iam_roles";
