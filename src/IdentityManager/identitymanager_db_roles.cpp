    ...
    return _parent->m_sqlConnector->query("INSERT INTO iam_roles (`roleName`,`roleDescription`) VALUES(:roleName,:roleDescription);",
                                          {
                                              ...
                                              {":roleDescription",MAKE_VAR(STRING,roleDescription)}
                                          });
}
...
{
    Threads::Sync::Lock_RW lock(_parent->m_mutex);
    return _parent->m_sqlConnector->query("DELETE FROM iam_roles WHERE `roleName`=:roleName;",
                               {
                                   {":roleName",MAKE_VAR(STRING,roleName)}
                               });
}
...
bool IdentityManager_DB::Roles_DB::doesRoleExist(const std::string &roleName)
{
    ...
    std::shared_ptr<SQLConnector::QueryInstance> i = _parent->m_sqlConnector->qSelect("SELECT `roleName` FROM iam_roles WHERE `roleName`=:roleName;",
                                          {{":roleName",MAKE_VAR(STRING,roleName)}},
                                          {});
...
std::set<std::string> IdentityManager_DB::Roles_DB::getRolesList()
{
    ...
    std::shared_ptr<SQLConnector::QueryInstance> i = _parent->m_sqlConnector->qSelect("SELECT `roleName` FROM iam_roles;",
                                          {},
                                          { &roleName });
