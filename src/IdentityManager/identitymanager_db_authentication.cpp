...
bool IdentityManager_DB::AuthController_DB::validateApplicationPermissionOnRole(const std::string &roleName, const ApplicationPermission &permission, bool lock)
{
    ...
    std::shared_ptr<SQLConnector::QueryInstance> i = _parent->m_sqlConnector->qSelect("SELECT `f_roleName` FROM iam_applicationPermissionsAtRole WHERE `f_permissionId`=:permissionId AND `f_appName`=:appName AND `f_roleName`=:roleName;",
...
bool IdentityManager_DB::AuthController_DB::changeCredential(const std::string &accountName, Credential passwordData, uint32_t slotId)
{
    ...
    // Destroy (if exist).
    _parent->m_sqlConnector->query("DELETE FROM iam_accountCredentials WHERE `f_userName`=:userName and `f_AuthSlotId`=:slotId",
                                   {
                                       {":userName",MAKE_VAR(STRING,accountName)},
                                       {":slotId",MAKE_VAR(UINT32,slotId)}
                                   });
    ...
    return _parent->m_sqlConnector->query("INSERT INTO iam_accountCredentials "
                                          "(`f_AuthSlotId`,`f_userName`,`hash`,`expiration`,`salt`,`forcedExpiration`,`usedstrengthJSONValidator`) "
                                          "VALUES"
...
void IdentityManager_DB::AuthController_DB::updateAccountLastLogin(const std::string &accountName, const uint32_t &slotId, const Sessions::ClientDetails &clientDetails)
{
    Threads::Sync::Lock_RW lock(_parent->m_mutex);

    // Intenta actualizar primero
    bool updated = _parent->m_sqlConnector->query(
        "UPDATE iam_accountsLastLog SET `lastLogin`=CURRENT_TIMESTAMP WHERE `f_username`=:userName;",
        { {":userName", MAKE_VAR(STRING,accountName)} }
        );
    ...
    if (!updated)
    {
        _parent->m_sqlConnector->query(
            "INSERT INTO iam_accountsLastLog(`f_username`, `lastLogin`) VALUES (:userName, CURRENT_TIMESTAMP);",
            { {":userName", MAKE_VAR(STRING,accountName)} }
            );
    }
    ...
    _parent->m_sqlConnector->query(
        "INSERT INTO iam_accountAuthLog(`f_userName`, `f_AuthSlotId`, `loginDateTime`, `loginIP`, `loginTLSCN`, `loginUserAgent`, `loginExtraData`) "
...
time_t IdentityManager_DB::AuthController_DB::getAccountLastLogin(const std::string &accountName)
{
...

    {
        ...
        std::shared_ptr<SQLConnector::QueryInstance> i = _parent->m_sqlConnector->qSelect(
            "SELECT `lastLogin` FROM iam_accountsLastLog WHERE `f_username`=:userName LIMIT 1;",
            { {":userName", MAKE_VAR(STRING,accountName)} },
            { &lastLogin }
            );
...
std::set<uint32_t> IdentityManager_DB::AuthController_DB::listUsedAuthenticationSlotsOnAccount(const std::string &accountName)
{
    ...
    std::shared_ptr<SQLConnector::QueryInstance> i = _parent->m_sqlConnector->qSelect("SELECT `f_AuthSlotId` FROM iam_accountCredentials WHERE `f_userName`=:f_userName;",
                                                                                      {  {":f_userName",             MAKE_VAR(STRING,accountName)} },
                                                                                      { &slotId });
...
uint32_t IdentityManager_DB::AuthController_DB::addNewAuthenticationSlot(const AuthenticationSlotDetails &details)
{
    ...
    auto i = _parent->m_sqlConnector->qInsert("INSERT INTO iam_authenticationSlots (`description`,`function`,`defaultExpirationSeconds`,`strengthJSONValidator`) VALUES(:description,:function,:defaultExpirationSeconds,:strengthJSONValidator);",
                                          {
...
{
    Threads::Sync::Lock_RW lock(_parent->m_mutex);
    return _parent->m_sqlConnector->query("DELETE FROM iam_authenticationSlots WHERE `slotId`=:slotId;",
                                          {
                                              {":slotId", MAKE_VAR(UINT32,slotId)}
                                          });
}
...
bool IdentityManager_DB::AuthController_DB::updateAuthenticationSlotDetails(const uint32_t &slotId, const AuthenticationSlotDetails &details)
{
    ...
    return _parent->m_sqlConnector->query(
        "UPDATE iam_authenticationSlots SET "
...
uint32_t IdentityManager_DB::AuthController_DB::addAuthenticationScheme(const std::string &description)
{
    ...
    auto i = _parent->m_sqlConnector->qInsert("INSERT INTO iam_authenticationSchemes (`description`) VALUES(:description);",
                                              {
...
bool IdentityManager_DB::AuthController_DB::updateAuthenticationScheme(const uint32_t &schemeId, const std::string &description)
{
    ...
    // Update...
    return _parent->m_sqlConnector->query(
        "UPDATE iam_authenticationSchemes SET "
        "`description` = :description "
...
{
    Threads::Sync::Lock_RW lock(_parent->m_mutex);
    return _parent->m_sqlConnector->query("DELETE FROM iam_authenticationSchemes WHERE `schemeId`=:schemeId;",
                                          {
                                              {":schemeId", MAKE_VAR(UINT32,schemeId)}
                                          });
}
...
std::map<uint32_t, std::string> IdentityManager_DB::AuthController_DB::listAuthenticationSchemes()
{
    ...
    std::shared_ptr<SQLConnector::QueryInstance> i = _parent->m_sqlConnector->qSelect("SELECT `schemeId`, `description` FROM iam_authenticationSchemes;",
                                                                                      { },
...
uint32_t IdentityManager_DB::AuthController_DB::getApplicationActivityDefaultScheme(const std::string &appName, const std::string &activityName)
{
    ...
    std::shared_ptr<SQLConnector::QueryInstance> i = _parent->m_sqlConnector->qSelect(
        "SELECT `defaultSchemeId` FROM iam_applicationActivities WHERE `f_appName`=:appName AND `activityName`=:activityName;",
        {
...
bool IdentityManager_DB::AuthController_DB::setApplicationActivityDefaultScheme(const std::string &appName, const std::string &activityName, const uint32_t &schemeId)
{
    ...
    return _parent->m_sqlConnector->query(
        "UPDATE iam_applicationActivities SET `defaultSchemeId`=:schemeId WHERE `f_appName`=:appName AND `activityName`=:activityName;",
        {
            {":schemeId", MAKE_VAR(UINT32,schemeId)},
            {":appName", MAKE_VAR(STRING,appName)},
            {":activityName", MAKE_VAR(STRING,activityName)}
...
std::set<uint32_t> IdentityManager_DB::AuthController_DB::listAuthenticationSchemesForApplicationActivity(const std::string &appName, const std::string &activityName)
{
    ...
    std::shared_ptr<SQLConnector::QueryInstance> i = _parent->m_sqlConnector->qSelect("SELECT `f_schemeId` FROM iam_applicationActivitiesAuthSchemes WHERE `f_appName`=:appName AND `f_activityName`=:activityName;",
                                                                                      {
                                                                                       {":appName", MAKE_VAR(STRING,appName)},
                                                                                       {":activityName", MAKE_VAR(STRING,activityName)}
                                                                                      },
...
bool IdentityManager_DB::AuthController_DB::updateAuthenticationSlotUsedByScheme(const uint32_t &schemeId, const std::list<AuthenticationSchemeUsedSlot> &slotsUsedByScheme)
{
    ...
    for (const auto& slot : slotsUsedByScheme) {
        std::string insertSql = "INSERT INTO `iam_authenticationSchemeUsedSlots` (`f_schemeId`, `f_slotId`, `orderPriority`, `optional`) VALUES (:schemeId, :slotId, :orderPriority, :optional);";
...
{
    Threads::Sync::Lock_RW lock(_parent->m_mutex);
    _parent->m_sqlConnector->query("UPDATE iam_accountCredentials SET `badAttempts`='0' WHERE `f_userName`=:userName and `f_AuthSlotId`=:slotId;",
                                   {
                                       {":userName",MAKE_VAR(STRING,accountName)},
                                       {":slotId",MAKE_VAR(UINT32,slotId)}
                                   });
...
{
    Threads::Sync::Lock_RW lock(_parent->m_mutex);
    _parent->m_sqlConnector->query("UPDATE iam_accountCredentials SET `badAttempts`=`badAttempts`+1  WHERE `f_userName`=:userName and `f_AuthSlotId`=:slotId;",
                                   {
                                       {":userName",MAKE_VAR(STRING,accountName)},
                                       {":slotId",MAKE_VAR(UINT32,slotId)}
                                   });
...
std::set<ApplicationPermission> IdentityManager_DB::AuthController_DB::getAccountDirectApplicationPermissions(const std::string &accountName, bool lock)
{
    ...
    std::shared_ptr<SQLConnector::QueryInstance> i = _parent->m_sqlConnector->qSelect("SELECT `f_appName`,`f_permissionId` FROM iam_applicationPermissionsAtAccount WHERE `f_userName`=:userName;",
                                                                                      { {":userName",MAKE_VAR(STRING,accountName)} },
                                                                                      { &appName,&permission });
    while (i->getResultsOK() && i->query->step())
    {
