...
void WebAdminMethods_Roles::validateApplicationScopeOnRole(void *context, APIReturn &response, const RequestParameters &request, ClientDetails &authClientDetails)
{
...
void WebAdminMethods_Roles::getRoleApplicationScopes(void *context, APIReturn &response, const RequestParameters &request, ClientDetails &authClientDetails)
{
...
void WebAdminMethods_Roles::getRoleInfo(void *context, APIReturn &response, const RequestParameters &request, ClientDetails &authClientDetails)
{
    ...
    auto directScopes = Globals::getIdentityManager()->authController->getRoleApplicationScopes(roleName);

    i = 0;
    std::set<std::string> applications;
    for (const auto &scope : directScopes)
    {
        if (applications.find(scope.appName) != applications.end())
            continue; // This application has been already mapped.

        applications.insert(scope.appName);
        payloadOut["applications"][i]["name"] = scope.appName;
        payloadOut["applications"][i]["description"] = Globals::getIdentityManager()->applications->getApplicationDescription(scope.appName);

        // Take the active application scope for this role:
        int x = 0;
        for (const auto &scope2 : directScopes)
        {
            if (scope.appName == scope2.appName)
            {
                payloadOut["applications"][i]["scopes"][x]["id"] = scope.id;
                payloadOut["applications"][i]["scopes"][x]["description"] = Globals::getIdentityManager()->authController->getApplicationScopeDescription(scope);
                x++;
            }
        }
        // Take the unused application scope for this role:

        x = 0;
        for (const ApplicationScope &scope3 : Globals::getIdentityManager()->authController->listApplicationScopes(scope.appName))
        {
            if (directScopes.find(scope3) == directScopes.end())
            {
                payloadOut["applications"][i]["scopesLeft"][x]["id"] = scope3.id;
                payloadOut["applications"][i]["scopesLeft"][x]["description"] = Globals::getIdentityManager()->authController->getApplicationScopeDescription(scope3);
                x++;
            }
        }
    ...
    {
        if (!WebAdmin_Methods::iScopesLeftListForRole(appName, roleName).empty())
        {
