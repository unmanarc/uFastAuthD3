...
function formatDateHTML(timestampMsOrNull) {
  if (!timestampMsOrNull) return '<span class="text-muted">N/A</span>';
  const d = new Date(timestampMsOrNull);
  if (isNaN(d.getTime())) return '<span class="text-muted">N/A</span>';
...
function formatDatePlain(timestampMsOrNull) {
  if (!timestampMsOrNull) return 'N/A';
  const d = new Date(timestampMsOrNull);
